...
<div class="col-md-12 col-sm-12 col-xs-12">
            ...
            <?php
                     ...
                     foreach (BroadCast::all() as $broadcast):
                 $date =  strtoupper( date("M j, Y", date_timestamp_get($broadcast->created_at)));
                  $content = substr($broadcast->content , 0 , 70);

if($broadcast->status == 0){

    $action = "  <a href='".$this->domain."/admin-broadcast/pausePlayBroadcast/".$broadcast->id."'>
    <button type='button' class='btn btn-primary btn-xs' >Pause</button></a>";
}else{
    $action = "  <a href='".$this->domain."/admin-broadcast/pausePlayBroadcast/".$broadcast->id."'>
      <button type='button' class='btn btn-success btn-xs'>Play</button></a>";
}
                   ...
                   ?>
                          ...
                          <td style="text-transform: capitalize;"><?=$broadcast->admin->lastname;?> <?=$broadcast->admin->firstname;?></td>
                          <td><?=$date;?></td>
                          <td>
<div class='btn-group btn-group-xs'>
  <button type='button' class='btn btn-danger' onclick="confirm_delete(<?=$broadcast->id;?> ,'<?=$broadcast->email?>' );" data-toggle='modal' data-target='#delete_newsletter_modal'><i class='fa fa-trash'></i></button>
<?=$action;?>
</div>
                          </td>
                          </tr>
...
<form method="post" action="<?=$this->domain;?>/admin-broadcast/broadCastMessage">


  <div class="col-lg-12 form-group has-feedback">
    <textarea class="form-control  has-feedback-left" name="broadcast_message" id="inputSuccess2" rows="3" placeholder="Brief content of Message" required=""></textarea>
