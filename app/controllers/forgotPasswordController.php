...
public function reset_password()
{
 ...
 /*echo*/	$user->update(['password'=>Input::get('new_password')]);
 	...
 	// print_r($this->validator->errors());
 		...
 		Session::putFlash('info' , 'Pls Try again!');
...
{
	$this->view('guest/auth/change-password', ['email'=> $email]);
...
public function send_link()
{
	...
	if ($user) {
		...
		$body = "<h1>".Config::project_name()."</h1>Pls click the link to reset your password <a href='".Config::domain()."/forgot-password/confirm_reset/$email/$token'>Reset Password</a>";
		...
		if ($mailer->sendMail($to, $subject, $body)) {
				...
				Session::putFlash('danger','Email does not match our record. ');
	}
	...
	{
		$this->view('guest/auth/forgot-password');
